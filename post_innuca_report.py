...
def analyse_report(report, mlst, nr_contigs, min_bps, max_bps, min_gc, max_gc):
...
def pilon_report_analysis(species_mlst, assemblies, output, pilon_report_path, mlst_report_path,
    ...
    # Analyse report
    result = analyse_report(final_report_dict, species_mlst, nr_contigs, min_bps,
                            max_bps, min_gc, max_gc)
...
def main(species, output, assemblies, pilon_report_path, mlst_report_path,
         cpu, nr_contigs, min_bps, max_bps, min_gc, max_gc, pfile):
...
#    print(species_dict)
    ...
    try:
        ...
        raise Exception("Invalid or unknown species. Please provide a valid species name.")

    if pfile is not False:
        with open(pfile, 'r') as f:
            parameters = f.readlines()
            parameters = [p.strip().split('\t') for p in parameters]
            parameters = {p[0]: [p[1], p[2], p[3], p[4]] for p in parameters[1:]}

        if parameters.get(species_mlst, 0) != 0:
            species_parameters = parameters[species_mlst]
            min_bps = int(species_parameters[0])
            max_bps = int(species_parameters[1])
            min_gc = float(species_parameters[2])
            max_gc = float(species_parameters[3])
    ...
    if assemblies:
        ...
        # Analyse results
        analysed_report = analyse_report(results, species_mlst, nr_contigs,
        ...
        report = report[['Sample', 'Number of contigs', 'Average contig size', 'N50',
                         'Total assembly length', 'GC content', 'Missing Data', 'mlst']]
...
def parse_arguments():
    ...
    parser.add_argument('--pf', type=str, required=False, dest='parameters_file',
                        default=False, help='File with total number of bases and GC content values '
                                            'to be considered for the input genomes/assemblies.')

    args = parser.parse_args()

    return [args.species, args.output_path, args.assembly_path, args.pilon_report_path,
            args.mlst_report_path, args.cpu, args.nr_contigs, args.minimum_number_of_bases,
            args.maximum_number_of_bases, args.minimum_gc_content, args.maximum_gc_content,
